...
function getReadinessLabel(level: string, locale: "en" | "uk") {
  switch (level) {
    case "leader":
      return t(locale, "assessments.readiness.leader")
    case "advanced":
      return t(locale, "assessments.readiness.advanced")
    case "developing":
      return t(locale, "assessments.readiness.developing")
    case "emerging":
      return t(locale, "assessments.readiness.emerging")
    default:
      return level
  }
}
...
export default async function AssessmentDetailPage({
  ...
  const { data: assessment } = await db
    ...
    .select()
    .eq("id", id)
    .eq("client_id", (client as unknown as Client | null)?.id)
    .single()
  ...
  const reportResults = dbAssessmentRowToResults(assessment as Record<string, unknown>, typedClient?.id ?? "")
      ...
      <div className="space-y-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{t(locale, "assessmentDetail.extendedOnly")}</p>
          </div>
        </div>
          ...
          <CardContent className="p-8">
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              ...
              <div className="max-w-md space-y-3">
                ...
                <p className="text-sm text-muted-foreground">{t(locale, "assessmentDetail.unlockDescription")}</p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground flex items-center gap-2">
              ...
              {new Date(typedAssessment.created_at).toLocaleDateString(dateLocale, {
                year: "numeric",
                month: "long",
                day: "numeric",
              })}
