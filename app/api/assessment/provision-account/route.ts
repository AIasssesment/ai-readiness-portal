  ...
  try {
    ...
    await sql.begin(async (tx) => {
      const users = await tx<Array<{ id: string }>>`
        insert into app_users (email, full_name, assessment_provision_expires_at)
        values (${email}, ${contactName}, ${expiresAt})
        returning id
      `
      ...
      await tx`
        ...
        values (${userId}, ${companyName}, ${contactName}, ${email})
      `
    })
    ...
    if (error instanceof Error && error.message.includes("assessment_provision_expires_at")) {
      return apiErrors.internal(
        "Database migration required: run scripts/009_app_users_assessment_provision.sql",
      )
    }
    return apiErrors.internal("Failed to create account")
  }
